...
playground:
	...
	@echo "==============================================================================="
	@if [ -z "$$FAL_KEY" ]; then \
		echo "⚠️  WARNING: FAL_KEY not set. Please export FAL_KEY='your-api-key'"; \
	fi
...
backend:
	...
	gcloud beta run deploy my-agentic-rag \
		--source . \
		--memory "4Gi" \
		--project $$PROJECT_ID \
		--region "us-central1" \
...
data-ingestion:
	...
	(cd data_ingestion && uv run data_ingestion_pipeline/submit_pipeline.py \
		...
		--region="us-central1" \
		--data-store-id="my-agentic-rag-datastore" \
		--data-store-region="us" \
		--service-account="my-agentic-rag-rag@$$PROJECT_ID.iam.gserviceaccount.com" \
		--pipeline-root="gs://$$PROJECT_ID-my-agentic-rag-rag" \
